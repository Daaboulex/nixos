# hardening — kernel/network sysctls, sudo (validated nrb-activate wrapper),
# polkit/rtkit. No AppArmor/MAC layer is configured.
{ inputs, ... }:
let
  mod =
    {
    ...
    in
    {
      ...
      config = lib.mkIf cfg.enable {
        ...
        # Reverse-path posture is LOOSE, stated once for both mechanisms that
        # implement it (the sysctl and the firewall's rpfilter fib check).
        # Strict breaks two deliberate asymmetric-routing designs: Mullvad's
        # policy routing, and split-tunnel alias replies (they arrive on the
        # tun still bearing the real remote source until the local-in un-DNAT).
        # Loose still drops sources with no route at all (true martians).
