...
let
  mod =
    ...
    let
      ...
      # Any-domain form: udev carries the domain through verbatim (lib/pci.nix).
      isPciAddr = myLib.pci.isValidPciAddrAnyDomain;
            ...
            lib.types.submodule {
              options = {
                ...
                extraGroups = lib.mkOption {
                  ...
                  default = [ ];
                  description = ''
                    Supplementary groups for the created user. Empty by default:
                    the seat's own devices are reachable via logind seat ACLs
                    (uaccess) and compositor TakeDevice — the classic
                    video/input/audio groups would instead grant access to EVERY
                    seat's device nodes ("input" alone reads the other seat's
                    keyboard), so add groups only for capabilities that genuinely
                    have no per-seat path (e.g. gamemode, uinput).
                  '';
                };
                cpuset = lib.mkOption {
