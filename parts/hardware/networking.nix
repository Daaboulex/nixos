  ...
  mod =
    ...
    in
    {
      ...
      options.myModules.hardware.networking = {
        ...
        vpnPlugins = lib.mkOption {
          type = lib.types.listOf lib.types.package;
          default = [ ];
          example = lib.literalExpression "[ pkgs.networkmanager-openvpn ]";
          description = ''
            NetworkManager VPN backend plugins to install (wires
            `networking.networkmanager.plugins`). Each plugin installs a VPN
            service NetworkManager discovers, so the KDE plasma-nm / nm-applet
            UI can import and manage that VPN type. Without the matching plugin
            KDE reports "NetworkManager is missing support for <type>". nixpkgs
            25.11 stopped shipping the VPN plugins by default, so they are now
            opt-in here -- e.g. `[ pkgs.networkmanager-openvpn ]` for OpenVPN.
          '';
        };
      ...
      config = lib.mkIf cfg.enable {
        networking = {
          ...
          # VPN backend plugins (e.g. OpenVPN). nixpkgs 25.11 dropped the default
          # plugin set, so KDE/plasma-nm reports "NetworkManager is missing
          # support for <type>" until the plugin is installed via this option.
          networkmanager.plugins = cfg.vpnPlugins;
        ...
        # Adding/removing a VPN plugin only drops a .name file into /etc; the
        # running NetworkManager daemon does not rescan it, so the new VPN type
        # stays invisible ("VPN service ... was not installed") until NM
        # restarts. Restart NM on the next switch whenever the plugin SET
        # changes, keyed on the stable plugin id (not the store path -> no
        # restart on version bumps): one brief reconnect on a plugin add/remove,
        # none otherwise. Empty list (no plugins) = no trigger, no restart.
        systemd.services.NetworkManager.restartTriggers = map (p: p.networkManagerPlugin) cfg.vpnPlugins;
