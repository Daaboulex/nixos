  ...
  mod =
    ...
    in
    {
      ...
      options.myModules.hardware.power = {
        ...
        tlpPd = lib.mkEnableOption "tlp-pd: TLP's power-profiles-daemon D-Bus interface so the KDE/GNOME power slider selects TLP's AC/Balanced/Save profile at runtime (requires tlp)";
      ...
      config = lib.mkIf cfg.enable {
        ...
        assertions = [
          ...
          {
            assertion = cfg.tlpPd -> cfg.tlp;
            message = "myModules.hardware.power: 'tlpPd' needs 'tlp' enabled (tlp-pd is TLP's ppd interface).";
          }
        ];
