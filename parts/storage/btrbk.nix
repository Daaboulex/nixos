...
#
# The module owns the replication domain: source snapshotting and
# replication (btrbk), the top-level source anchor mount
# (sourceAnchorDevice), and the verified restore path (the btrbk-restore
# command). The target drive's LUKS unlock, mount, keyfile permissions and
# scrub are owned by storage-crypt-drives; hosts declare the backup drive
# there. Hosts only parameterize.
{ inputs, ... }:
let
  mod =
    {
