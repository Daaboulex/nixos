...
let
  mod =
    ...
    let
      ...
      cryptDrives = config.myModules.storage.cryptDrives or { };
    in
    {
      _class = "nixos";
      ...
      config = lib.mkIf cfg.enable {
        ...
        assertions = [
          ...
          {
            assertion = lib.all (d: !lib.hasPrefix "/etc/" d.keyFile) (lib.attrValues cryptDrives);
            message = "myModules.boot.etcOverlay: a cryptDrives keyFile under /etc would be hidden by the overlay (the drive never unlocks) -- point myModules.storage.cryptDrives.<name>.keyFile at /var/lib/secrets";
          }
        ];
      };
    };
