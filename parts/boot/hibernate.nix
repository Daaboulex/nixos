      ...
      config = lib.mkIf cfg.enable {
        boot = {
          ...
          initrd.luks.devices.cryptswap = {
            ...
            allowDiscards = true;
            # Process dm-crypt inline instead of via the kcryptd workqueues:
            # pure I/O-path change (AES-256-XTS + Argon2 unchanged) that cuts
            # latency on the hibernate-image write at suspend and read at resume.
