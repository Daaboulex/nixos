    ...
    in
    {
      ...
      config = lib.mkIf cfg.enable {
        boot = {
          ...
          # Resume path. The mapper name appears only after initrd unlocks
          # the LUKS container, so kernel resume happens after LUKS setup.
          resumeDevice = "/dev/mapper/cryptswap";
          kernelParams = [ "resume=/dev/mapper/cryptswap" ];
        };
