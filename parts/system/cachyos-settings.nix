{ inputs, ... }: {
  flake.nixosModules.cachyos-settings = { config, lib, pkgs, ... }:
    let
      ...
      upstreamToggles = [
        "zram" "ioSchedulers" "audio" "storage" "thp" "systemd"
        "timesyncd" "networkManager" "ntsync" "debuginfod" "coredump"
        "nvidia" "amdgpuGcnCompat"
      ];

      # GPU-specific toggles default to false; all others default to true
      gpuToggles = [ "nvidia" "amdgpuGcnCompat" ];
    ...
    in {
      ...
      } // lib.genAttrs upstreamToggles (name: { enable = mkSubEnable name; })
        // {
        # --- Extra performance sysctls (NOT from CachyOS upstream) ---
        extraPerformance.enable = lib.mkEnableOption
          "Extra performance sysctls: BBR, cake, tcp_fastopen, buffer sizes, max_map_count, compaction, sched_autogroup"
          // { default = true; };
      };
