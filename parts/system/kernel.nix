  ...
  flake.nixosModules.system-kernel = { config, lib, pkgs, ... }:
    ...
    in {
      ...
      options.myModules.kernel = {
        ...
        channel = lib.mkOption { type = lib.types.enum [ "latest" "lts" "rc" ]; default = "latest"; description = "CachyOS kernel channel: latest (stable bleeding-edge), lts (long-term support), rc (release candidate)"; };
        ...
        cachyos = {
          ...
          bbr3 = lib.mkOption { type = lib.types.nullOr lib.types.bool; default = null; description = "BBR3 TCP congestion control"; };
          hzTicks = lib.mkOption { type = lib.types.nullOr lib.types.str; default = null; description = "Timer frequency (e.g. 1000, 500, 300)"; };
          kcfi = lib.mkOption { type = lib.types.nullOr lib.types.bool; default = null; description = "KCFI (Kernel Control Flow Integrity)"; };
          ...
          ccHarder = lib.mkOption { type = lib.types.nullOr lib.types.bool; default = null; description = "-O3 optimizations"; };
      ...
      config = lib.mkIf cfg.enable {
        boot.kernelPackages =
          let
            ...
            cachyosSuffix =
              if ch == "rc" then ""
              else (if v == "cachyos-lto" || v == "cachyos-sched-ext" then "-lto" else "") + cachyosArch;
            cachyosChannel =
              if ch == "rc" then "rc"
              else if ch == "lts" then "lts"
              else "latest";

            # Only pass defined inputs to the override
            cachyosOverrides = lib.filterAttrs (_: v: v != null) {
              inherit (cfg.cachyos) cpusched bbr3 hzTicks kcfi performanceGovernor tickrate preemptType ccHarder hugepage;
            };
            ...
            customCachyKernelPackages = if cachyosOverrides == {} then pkgs.cachyosKernels."linuxPackages-cachyos-${cachyosChannel}${cachyosSuffix}" else pkgs.linuxPackagesFor customCachyKernel;
          in
