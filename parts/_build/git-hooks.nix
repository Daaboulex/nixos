  ...
  perSystem =
    ...
    in
    {
      ...
      pre-commit.settings.hooks = standardHooks // {
        ...
        # client. Both sides are read from what myModules.nix.remoteBuilder
        # actually puts on disk -- /etc/nix/machines for a client, the
        # remotebuild user for a server -- so no host name is named here. A
        # client forces every build to a configured machine (--max-jobs 0); a
        # server builds locally; anything else SKIPS (never blocks, never
        # builds locally), as does a client whose machines are all unreachable.
        nix-spec-eval = {
          enable = true;
          name = "nix-spec-eval";
          entry = toString (
            (pkgs.writeShellApplication {
              ...
              text = ''
                ...
                builders="$(awk '!/^[[:space:]]*#/ && NF { sub(/^[a-z0-9+.-]*:\/\//, "", $1); sub(/^[^@]*@/, "", $1); print $1 }' /etc/nix/machines 2>/dev/null || true)"
                ...
                if [ -n "$builders" ]; then
                  ...
                  while read -r b; do
                    [ -n "$b" ] || continue
                    # Liveness only, and it must be a real SSH banner: a bare TCP
                    # connect is answered by middleboxes. Authenticating as the
                    # build user is impossible here (its key is root-owned and a
                    # git hook runs as the committer, so an ssh probe always
                    # denies -- which silently skipped this check forever); the
                    # daemon connects as root. Any one machine up is enough,
                    # because --max-jobs 0 lets nix choose among them all.
                    # No `producer | grep -q`: grep exits on match, the producer
                    # takes SIGPIPE and pipefail turns a hit into a miss.
                    # shellcheck disable=SC2016
                    banner="$(timeout 5 bash -c 'exec 3<>/dev/tcp/"$1"/22; head -c 8 <&3' _ "$b" 2>/dev/null || true)"
                    case "$banner" in
                      SSH-*)
                        found="$b"
                        break
                  ...
                  if [ -n "$found" ]; then
                    ...
                    echo "nix-spec-eval: no configured build machine reachable -- skipping (refusing a heavy local build on $self)."
                    exit 0
                  fi
                elif ! id -u remotebuild >/dev/null 2>&1; then
                  echo "nix-spec-eval: $self declares no build machine and is not a build server -- skipping (refusing a heavy local build)."
                  exit 0
                fi
