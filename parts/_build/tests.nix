    ...
    in
    {
      checks = {
        ...
        overlay-fixes-current =
          let
            probePkgs = import inputs.nixpkgs-unstable {
        ...
        # Lock node labels are traversal-assigned, not ownership: readers must
        # resolve root.inputs, and this gate pins the declared identity so any
        # new shadow, misnamed Daaboulex input, or stray full-nixpkgs instance
        # fails by name instead of waiting to mislead someone.
        lock-input-identity =
          let
            lock = builtins.fromJSON (builtins.readFile ../../flake.lock);
            rootInputs = lock.nodes.root.inputs;
            names = builtins.attrNames rootInputs;
            node = n: lock.nodes.${rootInputs.${n}};
            nameExceptions = [
              "openhmd-rift"
              "vkbasalt-overlay"
            ];
            badNames = builtins.filter (
              n:
              let
                l = (node n).locked or { };
              in
              (l.owner or "") == "Daaboulex"
              && !(builtins.elem n nameExceptions)
              && n != pkgs.lib.toLower (l.repo or "")
            ) names;
            declaredShadowed = [
              "flake-parts"
              "home-manager"
            ...
            shadowed = builtins.filter (n: rootInputs.${n} != n) names;
            declaredNixpkgs = [
              "nixpkgs"
              "nixpkgs-rocm6"
              "nixpkgs-unstable"
            ];
            nixpkgsInstances = builtins.filter (
              k:
              (lock.nodes.${k}.locked.repo or "") == "nixpkgs"
              && pkgs.lib.toLower (lock.nodes.${k}.locked.owner or "") == "nixos"
            ) (builtins.attrNames lock.nodes);
            sortEq = a: b: (builtins.sort builtins.lessThan a) == (builtins.sort builtins.lessThan b);
            problems =
              pkgs.lib.optional (
                badNames != [ ]
              ) "Daaboulex input name != lowercased repo name: ${toString badNames}"
              ++ pkgs.lib.optional (
                !sortEq shadowed declaredShadowed
              ) "shadowed root inputs now [${toString shadowed}], declared [${toString declaredShadowed}]"
              ++
                pkgs.lib.optional (!sortEq nixpkgsInstances declaredNixpkgs)
                  "full-nixpkgs instance nodes now [${toString nixpkgsInstances}], declared [${toString declaredNixpkgs}]";
          in
          if problems == [ ] then
            pkgs.runCommand "lock-input-identity" { } "echo 'OK: lock input identity holds'; touch $out"
          else
            throw "lock-input-identity: ${builtins.concatStringsSep " | " problems}";
        ...
        consumer-nixos-import =
          let
            testCfg = inputs.nixpkgs-unstable.lib.nixosSystem {
              inherit system;
              modules = [
