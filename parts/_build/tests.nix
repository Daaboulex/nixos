      ...
      checks = {
        ...
        eval-multiseat-collisions =
          let
            ...
            # Zero primaries: must be caught -- exactly one seat is the implicit seat0.
            zeroPrimarySeats = {
              a = mkSeat {
                user = "user";
                gpu = "0000:03:00.0";
          ...
          pkgs.runCommand "eval-multiseat-collisions"
            {
              ...
              zeroPrimary = builtins.concatStringsSep " | " (detect zeroPrimarySeats);
            }
            ''
              ...
              echo "zero-primary seats -> [$zeroPrimary]"
              grep -qi "isPrimary" <<< "$zeroPrimary" || { echo "FAIL: zero-primary not caught"; exit 1; }
              echo "OK: multiseat guard catches shared CPU/device/user/input + zero-primary, passes disjoint seats"
              touch $out
            '';
      };
    };
