# vfio-all — both VMs at once, thin headless iGPU host.
# Boot entry "ryzen-9950x3d-vfio-all". BOTH dGPUs are captured by vfio-pci at
# boot and both Windows VMs autostart: win11-amd on the RX 9070 XT (the main
# gaming monitor), win11-nvidia on the GTX 1660S (the vertical monitor, a 2nd
# person). Linux is HEADLESS on the iGPU (SSH + the iGPU TTY for recovery; no
# Linux login is needed to use either VM). The keyboard/mouse + GoXLR + Stream
# Deck are USB-passed into the amd VM; the SupremeFX + GREATHTEK port go to the
# nvidia VM. Whole-CCD-per-VM: 14 vCPU + 28 GB each, the 8th core of each CCD
# runs that VM's QEMU emulator/IO. Hugepages are boot-static (one pool can't be
# sized dynamically for two parallel VMs). On `systemctl poweroff` libvirt-guests
...
}:
{
  ...
  # nvidia drives nothing the host needs here (the 1660S is passed), so drop the
  # driver + blacklist nouveau → the card is unclaimed and vfio-pci.ids captures
  # it cleanly. amdgpu stays (it runs the iGPU host head). Mirrors vfio-nvidia.
  ...
  # restrictScxToHost is meaningless here (no spare host-only CCD — both are
  # guest-owned); the iGPU host floats on the freed emulator/IO threads.
  myModules.vfio.restrictScxToHost = lib.mkForce false;
  ...
  # 2×28 GiB = 56 × 1 GiB pages, reserved at boot (a single dynamic hook pool can't
  ...
  myModules.vfio.hugepages.count = lib.mkForce 56; # 2 × 28 × 1 GiB = 56 GiB
  ...
  # ── Thin-host service audit (§Q) — explicit mkForce false per line, no aggregate ──
