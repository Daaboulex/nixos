# _common-vfio — the passthrough machinery every VFIO spec shares.
# Imported by vfio-amd / vfio-nvidia / vfio-all; the _ prefix keeps it out of
...
{ config, lib, ... }:
{
  ...
  home-manager.users.${config.myModules.primaryUser}.myModules.home.lsfg-vk.enable =
    lib.mkForce false;
}
