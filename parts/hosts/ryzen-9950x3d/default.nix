...
}:
{
  ...
  myModules = {
    ...
    vfio = {
      ...
      # 0f:00.0 is the LUKS secondary data drive (/mnt/ssd, multi-purpose).
      protectedDiskAddrs = [
        "0000:04:00.0"
        "0000:0f:00.0"
      ];
      ...
      vms.win11-amd = {
        ...
        # NVMe passthrough: Windows = 0b:00.0 (Samsung 990 EVO Plus 1TB, PM9C1a
        # controller — Magician clone of the old 0f install: ESP+MSR+Windows+WinRE,
        # boots directly). NEVER 04:00.0 (system 9100 PRO) or 0f:00.0 (games 9100
        # PRO) — both in protectedDiskAddrs; the two identical 9100 PROs differ
        # only by serial+BDF and a bus renumber can swap them; the
        ...
        #
        # 0b shares IOMMU group 22 (chipset PCIe switch) with the NICs / SATA /
        # the 0f games NVMe. myModules.vfio.acsOverride
        # (set in the VFIO specialisations via _common-vfio.nix) splits the group
        # so ONLY this NVMe is passed; everything else stays on the host.
        pciPassthrough = [ "0000:0b:00.0" ]; # Windows NVMe (990 EVO Plus 1TB)
      ...
      vms.win11-nvidia = {
        ...
        # Disk: NONE — 0b (990 EVO Plus 1TB) is reassigned to win11-amd (the main
        # Windows clone lives there); this VM awaits a new dedicated NVMe (owner
        # decision) before it can boot Windows again. The
        # GREATHTEK USB controller (7c:00.4, isolated IOMMU grp 34) stays passed
        # WHOLE so arbitrary devices on its port reach the guest with native
        # hotplug (the 2nd-person swap port) — a hub passed by vendor:product
        # would NOT forward its children.
        pciPassthrough = [
  ...
  myModules.storage.compress = {
    enable = true;
    subvolumes = [ "/mnt/ssd/games" ];
  };

  # Root only; every cryptDrives mount adds itself. "/" covers all cryptroot subvolumes.
