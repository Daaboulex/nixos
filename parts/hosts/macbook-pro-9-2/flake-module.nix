...
#   hardware-rift-cv1
#   input-controllers input-ducky-one-x-mini input-ratbagd input-streamcontroller input-yeetmouse
#   nix-nix-ld
#   sensors-it87 sensors-msr sensors-nct6775 sensors-ryzen-smu sensors-zenpower
#   tuning-corecycler
#   vfio-base vfio-device-binding vfio-evdev vfio-hugepages vfio-kvmfr vfio-session-gpu vfio-vms
#
...
{ inputs, ... }:
{
  flake.nixosConfigurations.macbook-pro-9-2 = inputs.nixpkgs-unstable.lib.nixosSystem {
    specialArgs = {
      inherit inputs;
    ...
    modules = [
      ...
      (
        { config, ... }:
        {
          imports = [
            ...
            inputs.self.modules.nixos.storage-compress
            inputs.self.modules.nixos.storage-crypt-drives
            inputs.self.modules.nixos.storage-display-names
            ...
            inputs.openviking-nix.nixosModules.default
            inputs.lmstudio-nix.nixosModules.default
          ];
        }
      )
      ...
      inputs.home-manager.nixosModules.home-manager
      {
        ...
        home-manager.sharedModules = [
          inputs.lmstudio-nix.homeManagerModules.default
          inputs.rocksmith-nix.homeManagerModules.default
          inputs.free-claude-code-nix.homeManagerModules.default
        ];
      }
      ...
      # Overlays
      # Overlays — composed once in parts/overlays/_default.nix. Listing all
