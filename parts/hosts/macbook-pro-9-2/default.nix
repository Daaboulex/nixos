...
}:
{
  ...
  myModules = {
    ...
    storage = {
      ...
      fstrim.enable = true;
      # btrbk is configured by kingston.nix (enable = mkForce true +
    ...
    # --------------------------------------------------------------------------
    services = {
      # mDNS resolution moved to systemd-resolved (hardware.networking.multicastDns
    ...
    boot.kernel = {
      enable = true;
      # cachyos-lto built for x86-64-v2 (Ivy Bridge HD4000). v2 has no
      # upstream binary cache — kernel rotations compile from source.
      # When ryzen is reachable it builds via the remote-builder and MBP
      # just pulls; when ryzen is offline it falls back to a local 2C/4T
      # build (~3 hours). Bring ryzen up before a kernel bump if you want
      # a fast nrb.
      variant = "cachyos-lto";
