    ...
    in
    {
        ...
        client = {
          ...
          hostPublicKey = lib.mkOption {
            ...
            description = ''
              ...
              remote with `cat /var/lib/ssh/ssh_host_ed25519_key.pub`
              (pre-relocation hosts: /etc/ssh).
      ...
      config = lib.mkMerge [
        (lib.mkIf cfg.client.enable {
          ...
          # (Not max-jobs=0: that forces remote-only and hangs offline — the
          # hook emits `# postpone` indefinitely.)
          ...
          networking.hosts = lib.mkIf (cfg.client.staticIp != null) {
            # Pin only hostnames to the static IP, never a bare IP (a name that
            # is all digits and dots). isBareIp is the exact negation of the
            # keep set, written positively so the intent reads at a glance.
            ${cfg.client.staticIp} =
              let
                isBareIp = n: lib.hasInfix "." n && builtins.match "[0-9.]+" n != null;
              in
              lib.filter (n: !(isBareIp n)) ([ cfg.client.hostName ] ++ cfg.client.extraHostNames);
          };
        ...
        (lib.mkIf cfg.server.enable {
          # The server branch only contributes sshd settings (AllowUsers,
          # trusted-users) and the build user; security-ssh owns actually
          # running sshd. Make that hard dependency explicit rather than silent.
          assertions = [
            {
              assertion = config.myModules.security.ssh.enable;
              message = "remote-builder server requires myModules.security.ssh.enable -- it sets sshd AllowUsers/trusted-users but security-ssh owns running sshd.";
            }
          ];

          # Allow remotebuild user through sshd's AllowUsers gate.
          # Without this, ssh.nix's AllowUsers = [ primaryUser ] (root login is
          # disabled there) blocks the remotebuild user and all remote builds
          # fail silently with "failed to start SSH connection".
