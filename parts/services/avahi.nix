    ...
    in
    {
      ...
      config = lib.mkIf cfg.enable {
        services.avahi = {
          enable = true;
          nssmdns4 = true;
          # mDNS is for the LAN only — never advertise the hostname into the
          # Mullvad tunnel. It's a privacy leak (broadcasts our hostname to the
          # VPN) and the tunnel can reflect our own probe back, triggering an
          # endless name-conflict rename (host → host-2 → … → host-21).
          # Harmless on hosts that lack this interface.
