    ...
    programs.ghostty = myLib.mergeSettings {
      defaults = {
        ...
        settings = {
          ...
          copy-on-select = lib.mkDefault false; # zellij is the single clipboard owner; also kills the libadwaita "copied" toast
          ...
          # zellij owns scrollback; ghostty's is only a host fallback. 10 MB = upstream default.
          scrollback-limit = lib.mkDefault 10000000;

          # Fail-closed clipboard: confirm before pasting unsafe (newline-containing) content.
          clipboard-paste-protection = lib.mkDefault true;
          resize-overlay = lib.mkDefault "never"; # no size popup -- clean thin host

          font-size = lib.mkDefault 12;

          # --- Thin host: cede tabs + splits to zellij via selective unbind. NEVER a
          # blanket clear -- that drops ghostty's performable Escape/copy passthrough
          # (the scar). An unbound trigger passes through to zellij; copy/paste, font-size,
          # and Escape are deliberately kept. Triggers verified via `ghostty +list-keybinds`. ---
