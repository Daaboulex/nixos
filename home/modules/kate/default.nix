# kate — KDE text editor with theme-derived font and settings.
{
  config,
  lib,
  pkgs,
...
let
  cfg = config.myModules.home.kate;
  inherit (myLib.themeCtx { inherit config; }) hasTheme theme;
  themeFontFamily = theme.font.family;
in
{
  options.myModules.home.kate = {
    enable = lib.mkEnableOption "Kate KDE text editor";
  ...
  config = lib.mkIf cfg.enable {
    ...
    # ============================================================================
    programs.kate = myLib.mergeSettings {
      defaults = {
        enable = true;
        ...
        editor = {
          ...
          font = {
            family = lib.mkDefault (if hasTheme then themeFontFamily else "Hack");
