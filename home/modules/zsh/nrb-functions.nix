  ...
  nrb = ''
        nrb() {
          ...
          while [[ $# -gt 0 ]]; do
            case "$1" in
              ...
              --help|-h)
                ...
                echo "                       Use with --dry to preview; real sync keeps a remote backup"
          ...
          if [[ -n "$sync_target" ]]; then
            ...
            local _st="$sync_target" _st_ssh="" _try
            for _try in "$_st" "''${_st}.local"; do
            ...
            if (( dry )); then
              ...
              _rsync_args+=(--backup --backup-dir="../.nrb-sync-backups/nix-$_sync_stamp")
              _msg_step "Exact checksum-syncing flake to $_st_ssh"
            fi

            _msg_dim "  Source: $flake_dir/"
            _msg_dim "  Target: $_st_ssh:~/Documents/nix/"
            _msg_dim "  Safety: --checksum --delay-updates --delete-delay"
            if (( ! dry )); then
              _msg_dim "  Remote backup for overwritten/deleted files: ~/Documents/.nrb-sync-backups/nix-$_sync_stamp/"
            fi
