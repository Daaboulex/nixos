      ...
      defaults = {
        ...
        initContent = ''
          ...
          nrb() {
            ...
            local flake_dir="''${FLAKE_DIR:-${cfg.flakeDir}}"
            ...
            if [[ -n "$deploy_target" ]]; then
              ...
              for _try in "$_dt" "''${_dt}.local"; do
                if ssh -o ConnectTimeout=3 -o BatchMode=yes "$_try" true 2>/dev/null; then
                  _dt_ssh="$_try"
                  break
                fi
              done
            ...
            if (( update )); then
              ...
              _mk_kern_map() {
                ...
                nix --extra-experimental-features 'nix-command flakes' \
                  eval --json --impure --expr "$_mk_kern_expr" 2>>"$_eval_log"
              }
              _kern_map_eq() {
              ...
              _kern_is_cached() {
                ...
                for _sub in $(nix --extra-experimental-features 'nix-command flakes' \
                  ...
                  if ${pkgs.curl}/bin/curl -sfI "''${_sub}/''${_out_hash}.narinfo" >/dev/null 2>&1; then
                    return 0
                  fi
                done
                return 1
            ...
            if command -v git &>/dev/null && [[ -d "$flake_dir/.git" ]]; then
              ...
              if [[ -n "$dirty_files" ]]; then
                ...
                echo "$dirty_files" | while read -r f; do _msg_dim "  $f modified"; done
            ...
            if command -v nom &>/dev/null; then
              ...
              build_path=$(nix --extra-experimental-features 'nix-command flakes' \
                ...
                --print-out-paths --no-link \
                "''${_jobs_override[@]}" $trace_flag 2>&1 | tee /dev/stderr | grep '^/nix/store/' | tail -n1)
            fi
            ...
            case $switch_rc in
              ...
              4)  _msg_warn "Some systemd units failed to restart"
                  _msg_dim "  Check: systemctl --failed" ;;
