  ...
  config = lib.mkIf cfg.enable {
    ...
    programs.okular = myLib.mergeSettings {
      defaults = {
        ...
        # upstream evaluates `if showMenuBar` before its null-check, so the null
        # default crashes eval; pin okular's real default until that is fixed
        general.showMenuBar = lib.mkDefault true;
      };
