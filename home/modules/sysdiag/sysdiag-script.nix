...
# This file returns a function: { pkgs, colors? } -> script-string
{
  pkgs,
  colors ? null,
}:
...
''
  ...
  show_gpu() {
    ...
    ${pkgs.coreutils}/bin/timeout 5 ${pkgs.mesa-demos}/bin/glxinfo 2>/dev/null | head -50 || echo "  glxinfo not available"
  }
  ...
  show_display() {
    ...
    if [ "$SESSION_TYPE" = "wayland" ]; then
      ${pkgs.coreutils}/bin/timeout 3 ${pkgs.kdePackages.libkscreen}/bin/kscreen-doctor --outputs 2>/dev/null || ${pkgs.coreutils}/bin/timeout 3 ${pkgs.wlr-randr}/bin/wlr-randr 2>/dev/null || echo "  No Wayland monitor tool available"
    else
  ...
  show_overview() {
    ...
    df -h -x tmpfs -x devtmpfs -x efivarfs -x overlay 2>/dev/null | tail -n +2 | \
      ${pkgs.gawk}/bin/awk '!seen[$1]++ {printf "  %-20s %5s used of %5s (%s)\n", $6, $3, $2, $5}'
    ...
    if [ "$SESSION_TYPE" = "wayland" ]; then
      if command -v ${pkgs.kdePackages.libkscreen}/bin/kscreen-doctor &>/dev/null; then
        ${pkgs.coreutils}/bin/timeout 3 ${pkgs.kdePackages.libkscreen}/bin/kscreen-doctor --outputs 2>/dev/null | ${pkgs.gnused}/bin/sed 's/\x1b\[[0-9;]*m//g' | while read line; do
        ...
        ${pkgs.coreutils}/bin/timeout 3 ${pkgs.wlr-randr}/bin/wlr-randr 2>/dev/null
      fi
    fi
