...
# Returns a function: { colors? } -> script-string. Every command resolves
# via the runtimeDeps PATH the module pins (one mechanism, sudo-safe).
{
  colors ? null,
}:
let
  useTheme = colors != null;
  # Convert "R,G,B" to truecolor ANSI escape: \033[38;2;R;G;Bm
  tc = rgb: "\\033[38;2;${builtins.replaceStrings [ "," ] [ ";" ] rgb}m";
in
''
  #!/usr/bin/env bash

  # Colors${if useTheme then " (Breeze Dark truecolor)" else ""}
  ${
    if useTheme then
      ''
        RED='${tc colors.red-rgb}'
        GREEN='${tc colors.green-rgb}'
        YELLOW='${tc colors.orange-rgb}'
        BLUE='${tc colors.blue-rgb}'
        MAGENTA='${tc colors.purple-rgb}'
        CYAN='${tc colors.blue-alt-rgb}'
        WHITE='${tc colors.foreground-rgb}'
        GRAY='${tc colors.comment-rgb}'
      ''
    else
      ''
        RED='\033[0;31m'
  ...
  lspci 2>/dev/null | grep -qi "VGA.*AMD\|Display.*AMD\|3D.*AMD" && HAS_AMD_GPU=true
  lspci 2>/dev/null | grep -qi "VGA.*NVIDIA\|3D.*NVIDIA" && HAS_NVIDIA_GPU=true
  lspci 2>/dev/null | grep -qi "VGA.*Intel" && HAS_INTEL_GPU=true
  ...
  show_gpu() {
    ...
    lspci -v 2>/dev/null | grep -A 20 "VGA\|3D\|Display"
    ...
    timeout 5 glxinfo 2>/dev/null | head -50 || echo "  glxinfo not available"
  }
  ...
  show_display() {
    ...
    if [ "$SESSION_TYPE" = "wayland" ]; then
      timeout 3 kscreen-doctor --outputs 2>/dev/null || timeout 3 wlr-randr 2>/dev/null || echo "  No Wayland monitor tool available"
    else
  ...
  show_overview() {
    ...
    for gpu in /sys/class/drm/card[0-9]/device; do
      ...
      # Skip connector/writeback entries (card0-DP-1, card0-Writeback-2, etc.)
      PCI_ADDR=$(basename $(readlink -f $gpu) 2>/dev/null)
      GPU_NAME=$(lspci -s "$PCI_ADDR" 2>/dev/null | sed 's/.*: //' | cut -c1-50)
    ...
    # Storage summary
    section "💿 STORAGE"
    # Deduplicate btrfs subvolumes (same device = same usage, show mount point only once)
    df -h -x tmpfs -x devtmpfs -x efivarfs -x overlay 2>/dev/null | tail -n +2 | \
      awk '!seen[$1]++ {printf "  %-20s %5s used of %5s (%s)\n", $6, $3, $2, $5}'
    ...
    if [ "$SESSION_TYPE" = "wayland" ]; then
      if command -v kscreen-doctor &>/dev/null; then
        timeout 3 kscreen-doctor --outputs 2>/dev/null | sed 's/\x1b\[[0-9;]*m//g' | while read line; do
        ...
        timeout 3 wlr-randr 2>/dev/null
      fi
    fi
    ...
    systemctl list-units --type=service --state=running --no-pager --no-legend 2>/dev/null | \
      ...
      grep -E '^(NetworkManager|bluetooth|sddm|lact|scx|docker|libvirtd|sshd|tailscale|syncthing|cups|pipewire|wireplumber|lactd)$' | \
    ...
    if [ "$DMESG_ERR" -gt 0 ] || [ "$JOURNAL_ERR" -gt 0 ]; then
      ...
      journalctl -b -p err --no-pager -o cat 2>/dev/null | \
        sed 's/\[.*\]//g' | sort | uniq -c | sort -rn | head -5 | \
    ...
    # USB devices
    section "🔌 USB DEVICES"
    lsusb 2>/dev/null | grep -v "root hub" | while read line; do
    ...
    if $HAS_BATTERY; then
      ...
      if command -v upower &>/dev/null; then
        upower -i /org/freedesktop/UPower/devices/battery_BAT0 2>/dev/null | grep -E "state|percentage" | head -2
      fi
    fi
