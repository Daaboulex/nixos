...
in
{
  options.myModules.home.goxlr.eq = {
    ...
    # One block per GoXLR channel — generated from the channel->HiFi-sink
    # map, so the five blocks cannot drift apart. Option paths are unchanged
    # (channels.<name>.{enable,sink,eq}).
    channels =
      lib.mapAttrs
        (channel: sinkName: {
          enable = lib.mkEnableOption "EQ for ${channel} channel" // {
            default = true;
          };
          sink = lib.mkOption {
            type = lib.types.str;
            default = "alsa_output.usb-TC-Helicon_${deviceName}-00.HiFi__${sinkName}__sink";
            description = "PipeWire sink node name for ${channel} channel";
          };
          eq = lib.mkOption {
            type = lib.types.str;
            default = dt990proEqFilters;
            description = "PipeWire filter-chain EQ filter definition for ${channel} channel";
          };
        })
        {
          system = "Speaker";
