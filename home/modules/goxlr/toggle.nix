    ...
    home.packages = [
      (pkgs.writeShellScriptBin "goxlr-toggle" ''
        ...
        if [ "$current" = "${cfg.sleepProfile}" ]; then
          goxlr-client profiles device load "${cfg.activeProfile}"
          goxlr-client profiles microphone load "${cfg.activeMicProfile}"
          echo "GoXLR -> Active (${cfg.activeProfile})"
        else
          goxlr-client profiles device load "${cfg.sleepProfile}"
          goxlr-client profiles microphone load "${cfg.sleepMicProfile}"
