...
let
  ...
  standardIgnorePatterns = {
    documents = [
      ...
      # ── Git internals — NEVER sync .git/ (history travels via git remotes) ──
      # Bidirectionally syncing git refs/objects RACES and corrupts the repo:
      # sync-conflict refs (refs/heads/main.sync-conflict-*) → "invalid sha1
      # pointer" / "bad object HEAD" — even single-user, because a ref moves on
      # one host while its objects/refs are still mid-flight to the other. The
      # old "sync .git but exclude transient files" scheme did exactly that to
      # ryzen. Each host keeps its own .git and shares history through the
      # GitHub remotes (git push/pull) — what git is actually for. Repos with no
      # remote (e.g. site) are seeded once via rsync, then committed locally.
      "**/.git/"
