    ...
    programs.starship = myLib.mergeSettings {
      defaults = {
        ...
        // lib.optionalAttrs hasTheme {
          ...
          character = {
            success_symbol = "[>](bold green)"; # vi insert mode (and last cmd ok)
            error_symbol = "[>](bold red)"; # vi insert mode (and last cmd failed)
            vicmd_symbol = "[<](bold yellow)"; # vi normal/command mode indicator
          };
          directory = {
            style = "bold blue";
            truncation_length = 3;
