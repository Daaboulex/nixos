...
let
  ...
  # `or "nixos"` honors the osConfig ? { } default (standalone HM, no NixOS):
  # the lone bare osConfig read in home/modules otherwise throws there.
  detectedHost = osConfig.networking.hostName or "nixos";
in
{
  imports = [
    ./ui.nix
  ...
  options.myModules.home.neovim = {
    ...
    nixdFlakeDir = lib.mkOption {
      type = lib.types.str;
      description = "Flake directory nixd evaluates for option completions. The live tree, not `inputs.self`: a store snapshot would embed the config's own source path in init.lua (rebuilding it on every commit) and pin completions to a stale copy. Host-specific: set per-host in the home config.";
    };
  ...
  config = lib.mkIf cfg.enable {
    xdg.desktopEntries.nvim = {
      ...
      exec = "nvim %F";
      terminal = true; # launch in the user's terminal (KDE default terminal), not the neovide GUI
      icon = "nvim";
      categories = [
        "Utility"
        "TextEditor"
    ...
    programs.neovim = myLib.mergeSettings {
      defaults = {
        ...
        viAlias = lib.mkDefault true; # vi -> nvim
        ...
        plugins = [
          ...
          (pkgs.vimPlugins.nvim-treesitter.withPlugins (
            ...
            ++ lib.optionals cfg.lsp.rust [ p.rust ]
            ++ lib.optionals cfg.lsp.go [ p.go ]
          ))
        ...
        initLua = ''
          ...
          -- ============================================================
          -- Smooth scrolling (neoscroll) -- animated <C-d>/<C-u>/<C-f>/<C-b>/zz;
          -- the portable part of Neovide's scroll feel, inside the TUI
          -- ============================================================
          require('neoscroll').setup({})

          -- ============================================================
          -- Flash (label-jump motion). char=true enhances f/F/t/T with labels;
          -- search left vanilla. Explicit jumps under <leader>j; s/S kept free.
          -- ============================================================
          require('flash').setup({
            modes = {
          ...
          -- ============================================================
          -- Secret files: no on-disk leakage for sensitive paths. Full-disk
          -- encryption only covers data at rest; this keeps file content out of
          -- ~/.local/state/nvim (undo/swap/backup) while the system is unlocked.
          -- In-memory undo is preserved; only the persisted copies are disabled.
          -- ============================================================
          vim.api.nvim_create_autocmd({ 'BufReadPre', 'BufNewFile' }, {
            pattern = {
              '*.age', '*.gpg', '*.asc', '*.pem', '*.key', '*.p12', '*.pfx',
          ...
          require('which-key').add({
            ...
            -- Jump (flash) -- normal/visual/operator-pending
            { '<leader>j', function() require('flash').jump() end, desc = 'Flash jump', mode = { 'n', 'x', 'o' } },
            { '<leader>J', function() require('flash').treesitter() end, desc = 'Flash Treesitter select', mode = { 'n', 'x', 'o' } },

            -- Cheat sheet + searchable discovery
            ...
            -- File manager (yazi) — only when the yazi module is enabled
            ...
            ${lib.optionalString config.myModules.home.lazygit.enable ''
              { '<leader>gl', function()
            ...
            ${lib.optionalString cfg.lsp.enable "{ '<leader>lf', function() require('conform').format({ timeout_ms = 500 }) end, desc = 'Format' },"}
