...
in
{
  ...
  options.myModules.home.neovim = {
    ...
    nixdFlakeDir = lib.mkOption {
      type = lib.types.str;
      default = "/home/user/Documents/nix";
      description = "Flake directory nixd evaluates for option completions. The live tree, not `inputs.self`: a store snapshot would embed the config's own source path in init.lua (rebuilding it on every commit) and pin completions to a stale copy.";
    };
