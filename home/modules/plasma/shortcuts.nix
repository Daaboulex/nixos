# shortcuts — Plasma keyboard shortcut bindings.
{
  config,
  lib,
  myLib,
...
let
  cfg = config.myModules.home.plasma.shortcuts;
  term = config.myModules.home.plasma.defaultTerminal;
in
{
  options.myModules.home.plasma.shortcuts = {
    enable = lib.mkEnableOption "Plasma keyboard shortcuts";
    ...
    programs.plasma = myLib.mergeSettings {
      defaults = {
        ...
        shortcuts = {
          ...
          # KDE-bundled apps (dolphin/krunner/spectacle/systemmonitor/emojier) ship
          # with Plasma, so referencing them here is safe. The terminal is the only
          # enable-gated one — bound below via plasma.defaultTerminal, never hardcoded.
        ...
        # Terminal launch — bound to plasma.defaultTerminal (kdeglobals counterpart
        # in appearance.nix); self-selects ghostty > konsole so it never dangles.
        // lib.optionalAttrs (term != null) {
          "${term}"."_launch" = "Ctrl+Alt+T";
        };
      };
