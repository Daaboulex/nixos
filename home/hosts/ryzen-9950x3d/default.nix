...

{
  ...
  myModules.home = {
    ...
    neovim.nixdFlakeDir = "${config.home.homeDirectory}/Documents/nix";
    ...
    plasma.panels.hiddenTrayItems = [ "plasmashell_microphone" ];
    ...
    cod-clients.cblauncher.gameDirs = [ "/mnt/ssd/games/CoD" ];
  ...
  # protontricks is used from the command line only; its GUI is a bundled GTK
  # yad dialog that renders unstyled and slow on this Wayland Plasma session.
  # A user entry of the same id shadows the package's, hiding it from the
  # launcher without patching the package.
  xdg.desktopEntries = {
    protontricks = {
      name = "Protontricks";
