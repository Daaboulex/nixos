...
The `boot-impermanence` module enables ephemeral root — `/` is rolled back to a blank BTRFS snapshot on every boot. Only explicitly declared state persists. This forces all system state to be reproducible.
...
The module automatically persists: `/var/lib/nixos`, `/var/lib/bluetooth`, `/var/lib/flatpak`, `/var/lib/NetworkManager`, `/var/lib/portmaster`, `/var/lib/sbctl`, `/etc/ssh`, `/etc/machine-id`, and more. Add extra paths via `myModules.boot.impermanence.extraDirectories`.
...
### Set Up Secrets (agenix)

Secrets are encrypted to each host's SSH public key. No separate age key needed.

```bash
# 1. Get this host's public key (NixOS generates it during install)
cat /etc/ssh/ssh_host_ed25519_key.pub

# 2. Add the public key to secrets/secrets.nix in the repo
#    (map it to the secret files this host needs)

# 3. Copy .age files to this host via Syncthing or manual transfer
#    (.age files are gitignored — they sync between hosts via Syncthing)

# 4. Rebuild to activate secret decryption
nrb
```
