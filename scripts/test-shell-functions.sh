...
for name in "${CONFIG_NAMES[@]}"; do
  ...
  KV=$(nix --extra-experimental-features 'nix-command flakes' \
    eval "$FLAKE_DIR#nixosConfigurations.$name.config.myModules.boot.kernel.variant" --raw 2>/dev/null || echo "")
  if [[ -n $KV ]]; then
...
# Check BUILD.md documents nrb-check (README was restructured to point at BUILD.md
# for full operator reference — see commit b381c67).
if grep -q "nrb-check" "$FLAKE_DIR/docs/BUILD.md"; then
  pass "docs/BUILD.md documents nrb-check"
else
  fail "docs/BUILD.md does not document nrb-check"
fi
