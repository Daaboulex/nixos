...
eval)
  ...
  hosts=""
  hosts_err=$(mktemp)
  if ! hosts=$(nix eval --raw ".#nixosConfigurations" \
    --apply 'hs: builtins.concatStringsSep " " (builtins.attrNames hs)' "${stub[@]}" 2>"$hosts_err"); then
    printf '  %-25s FAIL\n' "host enumeration"
    tail -5 "$hosts_err" | sed 's/^/    /'
    hosts=""
    failed=1
  fi
  rm -f "$hosts_err"
  for host in $hosts; do
    printf '  %-25s ' "$host"
    if [ "$host" = ryzen-9950x3d ]; then
      attr="system.build.etc.drvPath"
      label="OK (etc)"
    else
      attr="system.build.toplevel.drvPath"
      label="OK"
    fi
    if output=$(nix eval ".#nixosConfigurations.$host.config.$attr" "${stub[@]}" 2>&1); then
      echo "$label"
    else
      echo "FAIL"
