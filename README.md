...
nrb --install <host> root@<ip>   # bare-metal (re)install via nixos-anywhere (disko + escrowed identity)
...
[agenix](https://github.com/ryantm/agenix), encrypted to each host's SSH host key. The encrypted `*.age` blobs and the agenix recipient rules live in the private `site` registry (`repos/site/secrets/`, never pushed) — `myModules.security.agenix.secretsRoot` defaults to `inputs.site + /secrets`. Nothing secret lives in this public repo. Each host's identity (SSH host key) is additionally escrowed age-encrypted in the site registry, so `nrb --install` can reinstall a host that decrypts its secrets and keeps its registry pins from first boot.

## site input

`repos/site` is a private, local-only git repo (mirrored between hosts by `nrb --sync`) holding fleet identity (per-host IP/SSH keys/machine-ids/syncthing ids), the agenix secrets, and the identity escrow blobs. It is git-ignored here and never pushed, so the `site` flake input fetches it as its own git repo (`git+file://…/repos/site`) rather than a relative path. The path is valid on every fleet host; a host with a different `$HOME` rebuilds via `nrb`, which supplies `--override-input site …` automatically. A clone without `site` cannot evaluate the hosts that consume it.
