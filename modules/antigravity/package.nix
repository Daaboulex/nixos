...
let
  ...
  browserPkg =
    ...
    else if ungoogled-chromium != null then ungoogled-chromium
    else null;

  browserCommand =
    if isAarch64 then "chromium"
    else if google-chrome != null then "google-chrome-stable"
    else if ungoogled-chromium != null then "chromium"
    else null;

  browserProfileDir =
    if isAarch64 then "$HOME/.config/chromium" else "$HOME/.config/google-chrome";

  arch = if isAarch64 then "linux-arm64" else "linux-x64";
  
  src = fetchurl {
    url = "https://edgedl.me.gvt1.com/edgedl/release2/j0qc3/antigravity/stable/${version}/${arch}/Antigravity.tar.gz";
